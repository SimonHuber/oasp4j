...
public class SalesmanagementHttpRestServiceTestCitrus extends AbstractRestServiceTest {

  private final HttpHeaders AUTHENTIFICATED_HEADERS = getAuthentificatedHeaders();

  private static Logger LOG = LoggerFactory.getLogger(SalesmanagementHttpRestServiceTestCitrus.class);
  ...
  public void getOrder() {
    ...
    send("salesmanagementClientGet").http().method(HttpMethod.GET)
        .header(HttpHeaders.AUTHORIZATION, getAuthentificatedHeaders())
        .path(this.port + "/services/rest/salesmanagement/v1/order/" + responseOrderCto.getOrder().getId());
    receive("salesmanagementClientGet").messageType(MessageType.JSON).http().status(HttpStatus.OK);
  ...
  private String getAuthentificatedHeadersCitrus() {
